...
cormat_long <- as.data.frame(cormat) %>%
  ...
  pivot_longer(!index, names_to = 'pair', 
               values_to = 'cor') %>%
  filter(cor < 1) %>%
...
ms <- ms %>%
  filter(INDEX %in% 
           unique(cormat_long$index))
tm_shape(ms) + 
  tm_dots(col = 'ADM', legend.show = F)
# матрица расстояний
ms_dist <- st_distance(ms)
ms_dist
class(ms_dist)
typeof(ms_dist)
...
# выделяем классы расстояний и осредняем в их пределах
p2 <- df_swe %>%
  mutate(dist_class = cut(dist, breaks = seq(0, 8000, 100), 
                          labels = seq(0, 7900, 100))) %>%
  group_by(dist_class) %>%
  summarise(cor = mean(cor, na.rm = T)) %>%
  mutate(dist_class = as.integer(as.character(dist_class))) %>%
  ggplot(aes(x=dist_class, y=cor)) + 
  geom_point() + geom_smooth(se = F) + ylim(0, 1) +  
  scale_x_continuous(breaks = seq(0, 8000, 100), limits = c(0, 1000)) + 
  labs(x = 'Расстояние, км', y = 'R', title = 'Влагозапас в снеге')
p2


grid.arrange(p1, p2)

# нанесем уровень затухания до 1/e
p1 <- p1 + geom_hline(aes(yintercept = 1/exp(1)), linetype = 2)
p2 <- p2 + geom_hline(aes(yintercept = 1/exp(1)), linetype = 2)
